...
from stix2 import Bundle, parse, AttackPattern, Campaign, CourseOfAction, Identity, Indicator, Infrastructure, IntrusionSet, Location, Malware, Note, ObservedData, Report, ThreatActor, Tool, Vulnerability # Added specific imports
from stix2.exceptions import InvalidValueError # For catching stix2 validation errors
import uuid
import json
...
import io # Used to handle the uploaded file buffer
import time # Can be used for minor delays if needed, but mostly removed
from datetime import datetime, time as dt_time, date # For timestamp validation potentially
from dateutil.parser import isoparse # For robust ISO timestamp parsing
import urllib.parse # For URL encoding the maps search term
...
/* Add specific styling for the edit form section */
.edit-form-container {
...
.edit-form-container .stButton button {
    background-color: #28a745; /* Green update button */
    color: white;
}
.edit-form-container .stButton button:hover {
    background-color: #218838;
    color: white;
}
/* Style edit relationship form */
.edit-rel-form-container {
    border: 1px solid #ffc107; /* Yellow border for relationships */
    border-radius: 5px;
    padding: 15px;
    margin-top: 5px;
    background-color: #fff8e1; /* Light yellow background */
}
.edit-rel-form-container .stButton button {
    background-color: #ffc107; /* Yellow update button */
    color: #212529; /* Dark text */
    border-color: #ffc107;
}
.edit-rel-form-container .stButton button:hover {
    background-color: #e0a800;
    border-color: #d39e00;
    color: #212529;
}
/* Add some margin below subheaders inside forms */
.stForm h3 {
    margin-top: 1rem;
    margin-bottom: 0.5rem;
}
/* Ensure columns in forms have some gap */
.stForm [data-testid="column"] {
    padding-left: 5px;
    padding-right: 5px;
}
/* Style for the maps link */
.maps-link-container {
    margin-top: 10px;
    padding: 10px;
...
# Map STIX types to their respective classes from the stix2 library
TYPE_TO_CLASS_MAPPING = {
    ...
    # Add mappings for SCOs and other SDOs/SROs if you plan to create them via form
}

# --- Helper Functions for Form Generation and Parsing ---

def _get_default(data, key, default=""):
    """Helper to safely get values from object data, handling None."""
    val = data.get(key, default)
    return val if val is not None else default

def _list_to_text(data, key):
    """Convert list property to newline-separated string for text_area."""
    return "\n".join(data.get(key, []))

def _kill_chain_to_text(data, key):
    """Convert kill_chain_phases list to newline-separated string."""
    phases = data.get(key, [])
    return "\n".join([f"{p.get('kill_chain_name', '')}:{p.get('phase_name', '')}" for p in phases])

def _ext_ref_hashes_to_text(ext_refs):
    """Convert external reference hashes to text area format."""
    if not ext_refs or not isinstance(ext_refs, list) or not ext_refs[0].get("hashes"):
        return ""
    hashes_dict = ext_refs[0].get("hashes", {})
    return "\n".join([f"{algo}:{h}" for algo, h in hashes_dict.items()])

def _parse_stix_timestamp(timestamp_str):
    """Safely parse a STIX timestamp string into date and time objects."""
    if not timestamp_str or not isinstance(timestamp_str, str):
        return None, None
    try:
        dt_obj = isoparse(timestamp_str)
        # Return date and time, handling potential timezone info (make naive for widgets)
        return dt_obj.date(), dt_obj.time().replace(tzinfo=None)
    except ValueError:
        # Handle cases where the timestamp might be invalid
        st.warning(f"Could not parse timestamp: {timestamp_str}. Please check format.")
        return None, None
...
def generate_object_form_fields(obj_type, obj_data=None, key_prefix="add"):
    ...
    if obj_type == 'attack-pattern':
        ...
        datetime_input("First Seen", "first_seen")
        datetime_input("Last Seen", "last_seen")
        form_values['c_objective'] = st.text_area("Objective", value=_get_default(data, 'objective'), key=f"{key_prefix}_c_objective")
    elif obj_type == 'course-of-action':
         st.write("_No specific properties beyond common ones._")
        ...
        patt_types = ["stix", "pcre", "sigma", "snort", "suricata", "yara"]
        form_values['ind_pattern_type'] = st.selectbox("Pattern Type (Required)", options=patt_types, index=patt_types.index(_get_default(data, 'pattern_type', "stix")), key=f"{key_prefix}_ind_patt_type")
        form_values['ind_pattern'] = st.text_area("Pattern (Required)", value=_get_default(data, 'pattern'), key=f"{key_prefix}_ind_pattern")
        datetime_input("Valid From", "valid_from", required=True)
        datetime_input("Valid Until", "valid_until")
        form_values['ind_kill_chain_phases_str'] = st.text_area("Kill Chain Phases (kill_chain_name:phase_name)", value=_kill_chain_to_text(data, 'kill_chain_phases'), key=f"{key_prefix}_ind_kill_chain")
    elif obj_type == 'infrastructure':
         infra_types = ["amplification", "anonymization", "botnet", "command-and-control", "exfiltration", "hosting-malware", "hosting-target-lists", "phishing", "reconnaissance", "staging", "unknown"]
         form_values['inf_infrastructure_types'] = st.multiselect("Infrastructure Types", options=infra_types, default=data.get('infrastructure_types', []), key=f"{key_prefix}_inf_types")
         ...
         form_values['inf_kill_chain_phases_str'] = st.text_area("Kill Chain Phases (kill_chain_name:phase_name)", value=_kill_chain_to_text(data, 'kill_chain_phases'), key=f"{key_prefix}_inf_kill_chain")
         datetime_input("First Seen", "first_seen")
         datetime_input("Last Seen", "last_seen")
        ...
        form_values['is_secondary_motivations'] = st.multiselect("Secondary Motivations", options=ATTACK_MOTIVATION_OV[1:], default=data.get('secondary_motivations', []), key=f"{key_prefix}_is_secondary_motivations")
         ...
         form_values['mal_kill_chain_phases_str'] = st.text_area("Kill Chain Phases (kill_chain_name:phase_name)", value=_kill_chain_to_text(data, 'kill_chain_phases'), key=f"{key_prefix}_mal_kill_chain")
         datetime_input("First Seen", "first_seen")
         datetime_input("Last Seen", "last_seen")
        ...
        datetime_input("First Observed", "first_observed", required=True)
        datetime_input("Last Observed", "last_observed", required=True)
        ...
        form_values['ta_personal_motivations'] = st.multiselect("Personal Motivations", options=ATTACK_MOTIVATION_OV[1:], default=data.get('personal_motivations', []), key=f"{key_prefix}_ta_personal_motivations")
         ...
         form_values['t_kill_chain_phases_str'] = st.text_area("Kill Chain Phases (kill_chain_name:phase_name)", value=_kill_chain_to_text(data, 'kill_chain_phases'), key=f"{key_prefix}_t_kill_chain")
...
def parse_form_data_to_stix_args(form_data, obj_type):
    """Converts raw form data into a dictionary suitable for stix2 constructors or updates.
       Handles combining date/time inputs into STIX timestamps.
    """
    # (Function remains the same as previous version)
    obj_args = {}
    ...
    def format_stix_timestamp(date_val, time_val):
        ...
        try:
            ...
            return combined_dt.strftime('%Y-%m-%dT%H:%M:%S') + ".000Z"
        except Exception as e:
    ...
    if obj_type == 'attack-pattern':
        ...
        kcp = parse_kill_chain(form_data.get('ap_kill_chain_phases_str', '')); obj_args.update({'kill_chain_phases': kcp} if kcp else {})
    elif obj_type == 'campaign':
        ...
        ts = format_stix_timestamp(form_data.get('valid_from_date'), form_data.get('valid_from_time')); obj_args.update({'valid_from': ts} if ts else {})
        ...
        kcp = parse_kill_chain(form_data.get('ind_kill_chain_phases_str', '')); obj_args.update({'kill_chain_phases': kcp} if kcp else {})
    elif obj_type == 'infrastructure':
        ...
        kcp = parse_kill_chain(form_data.get('inf_kill_chain_phases_str', '')); obj_args.update({'kill_chain_phases': kcp} if kcp else {})
        ts = format_stix_timestamp(form_data.get('first_seen_date'), form_data.get('first_seen_time')); obj_args.update({'first_seen': ts} if ts else {})
        ts = format_stix_timestamp(form_data.get('last_seen_date'), form_data.get('last_seen_time')); obj_args.update({'last_seen': ts} if ts else {})
    elif obj_type == 'intrusion-set':
        ...
        if form_data.get('loc_latitude') is not None: obj_args['latitude'] = form_data['loc_latitude']
        if form_data.get('loc_longitude') is not None: obj_args['longitude'] = form_data['loc_longitude']
        if form_data.get('loc_precision') is not None: obj_args['precision'] = form_data['loc_precision']
    elif obj_type == 'malware':
        ...
        kcp = parse_kill_chain(form_data.get('mal_kill_chain_phases_str', '')); obj_args.update({'kill_chain_phases': kcp} if kcp else {})
        ts = format_stix_timestamp(form_data.get('first_seen_date'), form_data.get('first_seen_time')); obj_args.update({'first_seen': ts} if ts else {})
        ts = format_stix_timestamp(form_data.get('last_seen_date'), form_data.get('last_seen_time')); obj_args.update({'last_seen': ts} if ts else {})
        ...
        if form_data.get('note_content'): obj_args['content'] = form_data['note_content']
        if form_data.get('note_authors_str'): obj_args['authors'] = parse_text_area(form_data['note_authors_str'])
        if form_data.get('note_object_refs_str'): obj_args['object_refs'] = parse_text_area(form_data['note_object_refs_str'])
    elif obj_type == 'observed-data':
        ...
        if form_data.get('rep_report_types'): obj_args['report_types'] = form_data['rep_report_types']
        ts = format_stix_timestamp(form_data.get('published_date'), form_data.get('published_time')); obj_args.update({'published': ts} if ts else {})
        if form_data.get('rep_object_refs_str'): obj_args['object_refs'] = parse_text_area(form_data['rep_object_refs_str'])
    elif obj_type == 'threat-actor':
        ...
        kcp = parse_kill_chain(form_data.get('t_kill_chain_phases_str', '')); obj_args.update({'kill_chain_phases': kcp} if kcp else {})
    ...
    if ext_ref_source and (ext_ref_desc or ext_ref_url or ext_ref_ext_id):
    ...
    elif not ext_ref_source:
         obj_args["external_references"] = None
    required_fields = {
        ...
        'report': ['report_types', 'published', 'object_refs'],
    ...
    for key, value in args_copy.items():
        ...
        if key == 'is_family': continue
        is_timestamp_field = key in ['first_seen', 'last_seen', 'valid_until', 'published', 'first_observed', 'last_observed', 'valid_from']
        if not is_required and not is_timestamp_field and (value is None or value == "" or value == []):
            del obj_args[key]
        elif key == "external_references" and value is None:
            if key in obj_args: del obj_args[key]
        elif is_timestamp_field and value is None and key in obj_args:
             pass
    return obj_args
...
# Placeholder for status messages outside the sidebar
...
with st.sidebar:
    ...
    with st.expander("Load STIX 2.1 Bundle", expanded=True):
        uploaded_file = st.file_uploader(
            "Upload STIX 2.1 JSON bundle",
            type=["json"],
            key="stix_uploader_widget"
        )
        ...
        process_button = st.button(
            ...
            disabled=(st.session_state.uploaded_file_state is None),
            key="load_process_button"
        )

    # --- Processing Logic (Moved outside expander) ---
    if st.session_state.get("load_process_button") and st.session_state.uploaded_file_state is not None:
        with st.status(f"Loading '{st.session_state.uploaded_file_state.name}'...", expanded=True) as status:
            ...
            try:
                ...
                status.write("🔄 Resetting current graph...")
                ...
                st.session_state.editing_relationship_id = None
                time.sleep(0.2)
                status.write(f"📖 Reading file '{current_uploaded_file.name}'...")
                ...
                try: bundle_content_str = bundle_content_bytes.decode('utf-8-sig')
                    ...
                    try: bundle_content_str = bundle_content_bytes.decode('utf-8')
                        ...
                        st.stop()
                time.sleep(0.2)
                status.write("🧩 Parsing JSON data...")
                try: bundle_json = json.loads(bundle_content_str)
                    ...
                    st.stop()
                time.sleep(0.2)
                status.write("🔬 Parsing STIX objects...")
                try:
                    ...
                    st.stop()
                time.sleep(0.2)
                status.write("🏗️ Building graph elements...")
                ...
                error_label = f"Unexpected Error: {e}"
                status.update(label=error_label, state="error", expanded=True)
                load_status_placeholder.error(f"❌ An unexpected error occurred: {e}")
    ...
    with st.expander("Add STIX Object", expanded=False):
        # (Add Object form logic remains the same)
        ...
        with st.form("add_object_detail_form", clear_on_submit=True):
            ...
            if add_obj_submit:
                ...
                name_required = add_obj_type in ['intrusion-set', 'threat-actor', 'malware', 'tool', 'vulnerability', 'campaign', 'identity', 'location', 'course-of-action', 'attack-pattern', 'report']
                if name_required and not add_form_data.get('name'): error_messages.append(f"Name is required for {add_obj_type}.")
                if add_obj_type == 'threat-actor' and not add_form_data.get('ta_threat_actor_types'): error_messages.append("Threat Actor Types is required.")
                if add_obj_type == 'indicator':
                     if not add_form_data.get('ind_pattern_type'): error_messages.append("Pattern Type is required.")
                     if not add_form_data.get('ind_pattern'): error_messages.append("Pattern is required.")
                     if not add_form_data.get('valid_from_date'): error_messages.append("Valid From Date is required.")
                if add_obj_type == 'note' and not add_form_data.get('note_content'): error_messages.append("Content is required.")
                if add_obj_type == 'note' and not add_form_data.get('note_object_refs_str'): error_messages.append("Object Refs is required.")
                if add_obj_type == 'observed-data':
                    if not add_form_data.get('first_observed_date'): error_messages.append("First Observed Date is required.")
                    if not add_form_data.get('last_observed_date'): error_messages.append("Last Observed Date is required.")
                    if not add_form_data.get('obs_object_refs_str'): error_messages.append("Object Refs is required.")
                if add_obj_type == 'report':
                    if not add_form_data.get('rep_report_types'): error_messages.append("Report Types is required.")
                    if not add_form_data.get('published_date'): error_messages.append("Published Date is required.")
                    if not add_form_data.get('rep_object_refs_str'): error_messages.append("Object Refs is required.")
                ext_ref_source = add_form_data.get('ext_ref_source_name')
                ext_ref_provided = ext_ref_source or add_form_data.get('ext_ref_description') or add_form_data.get('ext_ref_url') or add_form_data.get('ext_ref_external_id') or add_form_data.get('ext_ref_hashes_str')
                if ext_ref_provided and not ext_ref_source: error_messages.append("Ext Ref: Source Name required.")
                elif ext_ref_source and not (add_form_data.get('ext_ref_description') or add_form_data.get('ext_ref_url') or add_form_data.get('ext_ref_external_id')): error_messages.append("Ext Ref: Desc, URL, or External ID required.")
                if error_messages:
                ...
                if validation_passed:
                    ...
                    obj_args['id'] = obj_id
                    obj_args['allow_custom'] = True
                    try:
                        if add_obj_type in TYPE_TO_CLASS_MAPPING:
                            constructor = TYPE_TO_CLASS_MAPPING[add_obj_type]
                            stix_obj = constructor(**obj_args)
                            obj_dict = json.loads(stix_obj.serialize())
                            st.session_state.objects.append(obj_dict)
                            st.session_state.nodes.append({'data': { 'id': obj_id, 'label': obj_args.get('name', obj_id) }})
                            st.success(f"Created {add_obj_type}: {obj_args.get('name', obj_id)}")
                            ...
                            st.rerun()
                        else: st.error(f"Cannot create object: No constructor mapping for type '{add_obj_type}'.")
                    except InvalidValueError as ve: st.error(f"STIX Validation Error creating '{obj_args.get('name', obj_id)}': {ve}")
                    except Exception as creation_e: st.error(f"Error creating STIX object '{obj_args.get('name', obj_id)}': {creation_e}")


    # --- Create Relationship ---
    with st.expander("Create Relationship", expanded=False):
        # (Create Relationship logic remains the same)
        eligible_nodes = [o for o in st.session_state.objects if o.get('type') != 'relationship' and o.get('type') != 'marking-definition']
    ...
    with st.expander("Edit Selected Object", expanded=(st.session_state.editing_object_id is not None)):
        # (Edit Object form logic remains the same)
        if st.session_state.editing_object_id:
            ...
            if object_to_edit:
                ...
                st.info(f"Editing: {object_to_edit.get('name', st.session_state.editing_object_id)} ({edit_obj_type})")
                st.markdown('<div class="edit-form-container">', unsafe_allow_html=True)
                with st.form(f"edit_object_form_{st.session_state.editing_object_id}", clear_on_submit=False):
                    ...
                    if update_obj_submit:
                        validation_passed = True; error_messages = []
                        name_required = edit_obj_type in ['intrusion-set', 'threat-actor', 'malware', 'tool', 'vulnerability', 'campaign', 'identity', 'location', 'course-of-action', 'attack-pattern', 'report']
                        if name_required and not edit_form_data.get('name'): error_messages.append(f"Name required for {edit_obj_type}.")
                        if edit_obj_type == 'threat-actor' and not edit_form_data.get('ta_threat_actor_types'): error_messages.append("Threat Actor Types required.")
                        if edit_obj_type == 'indicator':
                            if not edit_form_data.get('ind_pattern_type'): error_messages.append("Pattern Type required.")
                            if not edit_form_data.get('ind_pattern'): error_messages.append("Pattern required.")
                            if not edit_form_data.get('valid_from_date'): error_messages.append("Valid From Date required.")
                        ext_ref_source = edit_form_data.get('ext_ref_source_name')
                        ext_ref_provided = ext_ref_source or edit_form_data.get('ext_ref_description') or edit_form_data.get('ext_ref_url') or edit_form_data.get('ext_ref_external_id') or edit_form_data.get('ext_ref_hashes_str')
                        if ext_ref_provided and not ext_ref_source: error_messages.append("Ext Ref: Source Name required.")
                        elif ext_ref_source and not (edit_form_data.get('ext_ref_description') or edit_form_data.get('ext_ref_url') or edit_form_data.get('ext_ref_external_id')): error_messages.append("Ext Ref: Desc, URL, or External ID required.")
                        if error_messages:
                            validation_passed = False
                            for msg in error_messages: st.error(msg)
                        if validation_passed:
                            try:
                                ...
                                if obj_index != -1:
                                    original_obj = st.session_state.objects[obj_index]
                                    updated_obj = original_obj.copy()
                                    updated_obj['modified'] = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ')
                                    for key, value in update_args.items(): updated_obj[key] = value
                                    for key in list(updated_obj.keys()):
                                         if key not in update_args and key not in ['id', 'type', 'spec_version', 'created', 'modified']:
                                             if key in edit_form_data and (edit_form_data[key] == "" or edit_form_data[key] == []): del updated_obj[key]
                                             elif key == "external_references" and update_args.get("external_references") is None: del updated_obj[key]
                                    ...
                                    if node_index != -1:
                                        new_label = validated_obj_dict.get('name', validated_obj_dict.get('value', st.session_state.editing_object_id))
                ...
                st.warning("Selected object not found in current data.")
                st.session_state.editing_object_id = None
        else:
    ...
    with st.expander("Edit Selected Relationship", expanded=(st.session_state.editing_relationship_id is not None)):
        # (Edit Relationship logic remains the same)
            ...
            if rel_to_edit and rel_to_edit.get('type') == 'relationship':
                ...
                if source_obj and target_obj:
                    ...
                    with st.form(f"edit_relationship_form_{st.session_state.editing_relationship_id}", clear_on_submit=False):
                        st.text_input("Source", value=source_label, disabled=True)
                        st.text_input("Target", value=target_label, disabled=True)
                        ...
                        if update_rel_submit:
                            try:
                                ...
                                if rel_obj_index != -1 and edge_index != -1:
                                    ...
                                    updated_rel_obj['modified'] = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ')
                    ...
                    st.warning("Could not find source or target object for the selected relationship.")
                    st.session_state.editing_relationship_id = None
            else:
                st.warning("Selected element is not a relationship or not found.")
                st.session_state.editing_relationship_id = None
        else:
...
if not elements:
    st.info("Load a STIX bundle or add objects to view the graph.")
    selected = None
else:
    selected = cytoscape(
...
# (Selection update logic remains the same)
current_selection_in_state = st.session_state.get('current_selection')
new_editing_object_id = None
new_editing_relationship_id = None
rerun_needed_for_edit_state_change = False
...
if selected is not None and selected != current_selection_in_state:
    ...
    selected_edges = selected.get("edges", [])
    if len(selected_nodes) == 1 and not selected_edges:
        ...
        if edge_id != st.session_state.editing_relationship_id:
            new_editing_relationship_id = edge_id
            new_editing_object_id = None
            rerun_needed_for_edit_state_change = True
